...
async function fetchPullRequest(value) {
  ...
  const [pull, reactions, issueComments, reviewComments, reviews] = await Promise.all([
    ...
    ghJson(`${root}/issues/${ref.number}/reactions?per_page=100`),
    ghJson(`${root}/issues/${ref.number}/comments?per_page=100`),
    ghJson(`${root}/pulls/${ref.number}/comments?per_page=100`),
    ghJson(`${root}/pulls/${ref.number}/reviews?per_page=100`)
  ]);
  const comments = [
...
module.exports = { discoverPullRequest, fetchPullRequest, parsePullRequestUrl, postPullRequestComment, reactionSummary };
