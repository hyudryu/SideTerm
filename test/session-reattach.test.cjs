...
test('reattachment survives a direct PTY exiting during resize', () => {
  let killCalls = 0;
  const session = {
    processHandle: {
      pid: 8,
      resize() { throw new Error('process already exited'); },
      kill() { killCalls += 1; }
    },
    rendererReplay: 'final output',
    cwd: '/workspace',
    shell: 'bash',
    tmux: null
  };
  ...
  let details;
  assert.doesNotThrow(() => {
    details = reattachSession('exiting-session', session, { cols: 90, rows: 25 });
  });
  assert.equal(session.rendererReplay, 'final output');
  assert.equal(killCalls, 0);
  assert.deepEqual(details, {
    id: 'exiting-session', pid: 8, cwd: '/workspace', shell: 'bash', resumed: false,
    reattached: true, persistent: false, serverScrollback: false
  });
});
...
test('main buffers output per session until the renderer-ready handshake flushes it', () => {
  ...
  assert.match(renderer, /details \|\|= await api\.createSession[\s\S]*?await api\.markRendererReady\(id\);/);
  ...
  assert.match(main, /exitDeliveryToken: exited\.exitDeliveryToken,\s*hostGeneration: exited\.hostGeneration,\s*outputRevision: exited\.outputRevision/);
...
test('session creation shares pending work and a close cancels before registration', () => {
  ...
  assert.match(renderer, /details \|\|= await api\.createSession[\s\S]*?if \(sessions\.get\(id\) !== session\) return session;[\s\S]*?await api\.markRendererReady\(id\);\s*if \(sessions\.get\(id\) !== session\) return session;/);
});
...
test('renderer durably saves a new session id before asking the PTY host to spawn it', () => {
  ...
  const spawn = renderer.indexOf('details ||= await api.createSession', persist);
  ...
  assert.match(renderer, /if \(persistedCheckpointCoversDelivery\(session, entry\)\) \{\s*try \{\s*await entry\.acknowledge\(\);\s*\} catch \{\s*unsatisfied\.push\(entry\);/);
  ...
  assert.match(renderer, /cwd: session\.cwd,\s*exited: Boolean\(session\.exitObserved\),\s*exitCheckpointConfirmed: Boolean\(session\.exitCheckpointConfirmed\),\s*history: terminalHistory\(session\.terminal\)/);
  ...
  assert.match(renderer, /exited: saved\.exited,\s*exitCheckpointConfirmed: saved\.exitCheckpointConfirmed,/);
  assert.match(renderer, /const restoredTerminalState = decodeTerminalState\(options\.terminalState\);[\s\S]*?details \|\|= await api\.createSession[\s\S]*?const canRestoreTerminalState = Boolean\(restoredTerminalState[\s\S]*?nextHostGeneration === savedHostGeneration[\s\S]*?details\.reattached \|\| details\.exited[\s\S]*?terminal\.write\(restoredTerminalState, resolve\)[\s\S]*?safeHistory/);
  assert.match(renderer, /let details = options\.exited \? await api\.restoreStoppedSession\(\{[\s\S]*?terminalStateCols: restoredStateCols,[\s\S]*?exitCheckpointConfirmed: session\.exitCheckpointConfirmed[\s\S]*?if \(details\?\.stopped\) \{[\s\S]*?terminal\.write\(stoppedOutput, resolve\)/);
  assert.match(preload, /restoreStoppedSession: \(options\) => ipcRenderer\.invoke\('terminal:restore-stopped'/);
  assert.match(preload, /cleanupStopped: \(id, hostGeneration, snapshot = \{\}, retainMobile = true\) => ipcRenderer\.invoke\('terminal:cleanup-stopped'/);
  assert.match(main, /async function restoreStoppedSession[\s\S]*?tmuxSessionExists\(tmux, tmuxSession\)[\s\S]*?return createSession\(options\)[\s\S]*?process\.platform === 'win32' && !confirmedCheckpoint[\s\S]*?return createSession\(options\)[\s\S]*?cleanupStoppedSession/);
  assert.match(main, /async function cleanupStoppedSession\(id, hostGeneration, snapshot = \{\}, \{ retainMobile = true \} = \{\}\)[\s\S]*?mobileExitedSessions\.set\(sessionId[\s\S]*?liveSession\.processHandle\?\.generation[\s\S]*?await exitedSession\.processHandle\.kill\(\)[\s\S]*?client\.cleanupExitedSession\(sessionId, generation\)/);
  ...
  assert.match(renderer, /session\.closing = true;\s*try \{[\s\S]*?await api\.close\(id\);[\s\S]*?session\.checkpointRetired = true;[\s\S]*?pendingTerminalCheckpointAcknowledgements\.splice/);
  assert.match(renderer, /session\.closing = true;\s*try \{\s*if \(session\.exited\) \{\s*await api\.cleanupStopped\(id, session\.hostGeneration, \{\}, false\);\s*\} else \{\s*await api\.close\(id\);/);
  ...
  assert.match(renderer, /api\.onExit\(\(\{[\s\S]*?session\.exitObserved = true;[\s\S]*?session\.exitCheckpointConfirmed = false;[\s\S]*?const acknowledge = async \(\) => \{[\s\S]*?session\.exitCheckpointConfirmed = true;[\s\S]*?await persistWorkspaceNow\(\{[\s\S]*?protectedCheckpointSessionIds: new Set\(\[id\]\)[\s\S]*?acknowledgeExit\(\);[\s\S]*?acknowledgeTerminalDataAfterCheckpoint\(session, hostGeneration, outputRevision, acknowledge\)/);
  assert.match(renderer, /catch \(error\) \{\s*if \(sessions\.get\(id\) !== session\) return session;\s*session\.exited = true;[\s\S]*?Could not start the shell/);
  const startupFailure = renderer.slice(
    renderer.indexOf('} catch (error) {', renderer.indexOf('async function addSession(')),
    renderer.indexOf('if (options.activate !== false)', renderer.indexOf('async function addSession('))
  );
  assert.doesNotMatch(startupFailure, /exitObserved/);
